...
try
{
    ...
    for (int i = 0; i < 3; i++)
        sacks.Add(await GenerateUniqueSackAsync(conn, now, i, "DEMO-LOC-HN-01"));
    ...
    // ── 3b. CHỌN SỐ LƯỢNG PALLET ĐỂ HIỂN THỊ MÃ QR ────────────────────────────
    Console.WriteLine();
    Console.WriteLine("╔══════════════════════════════════════════════════════════════╗");
    Console.WriteLine("║               📦  TẠO MÃ QR CHO PALLET CÓ SẴN               ║");
    Console.WriteLine("╚══════════════════════════════════════════════════════════════╝");
    
    Console.Write("\n  Nhập số lượng mã QR Pallet ngẫu nhiên muốn hiển thị (Mặc định 3): ");
    var countInput = Console.ReadLine()?.Trim();
    if (!int.TryParse(countInput, out int palletCountToGenerate) || palletCountToGenerate <= 0)
        palletCountToGenerate = 3;

    var randomPallets = await GetRandomPalletsAsync(conn, palletCountToGenerate, hubInfo.LocationId);
    Console.WriteLine($"  ✓ Đã lấy ngẫu nhiên {randomPallets.Count} pallet (trạng thái Empty) tại {hubInfo.LocationName}.\n");
    ...
    foreach (var p in randomPallets)
        p.QrSvg = GenerateQrSvg(p.PalletId);
    ...
    var previewHtml = HtmlBuilder.BuildPreviewHtml(tripId, tripQrPayload, tripQrSvg, qrExpiresAt,
        origin, hubInfo, sacks, randomPallets, now);
    ...
    try
    {
        ...
        foreach (var sack in sacks)
            await InsertSackAsync(conn, transaction, sack, tripId, hubInfo.LocationId, now);
    ...
    // 9. Mở trang kết quả
    var successHtml = HtmlBuilder.BuildSuccessHtml(tripId, sacks, tripQrPayload, tripQrSvg, origin, hubInfo, randomPallets, now);
...
static async Task<List<PalletInfo>> GetRandomPalletsAsync(NpgsqlConnection conn, int count, string locationId)
{
    var sql = @"SELECT p.pallet_id, z.zone_name, p.status, p.capacity, z.zone_id
                FROM pallet p
                JOIN zone z ON z.zone_id = p.zone_id
                WHERE z.location_id = @locationId AND p.status = 'Empty'
...
static async Task InsertSackAsync(NpgsqlConnection conn, NpgsqlTransaction tx,
    SackDraft sack, string tripId, string hubId, DateTime now)
{
    const string sql = @"
        INSERT INTO sack (sack_id, trip_id, status, created_at, s_destination, zone_id)
        VALUES (@sackId, @tripId, 'Sorting', @createdAt, @destination, 
                (SELECT zone_id FROM zone WHERE location_id = @hubId AND process_role = 'LocalSortBuffer' LIMIT 1))";
    await using var cmd = new NpgsqlCommand(sql, conn, tx);
...
sealed class PalletInfo(string palletId, string zoneName, string status, decimal capacity, string zoneId)
{
    public string PalletId { get; } = palletId;
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    private static string SackCards(List<SackDraft> sacks, bool success)
    {
        ...
        for (int i = 0; i < sacks.Count; i++)
        {
            ...
            sb.Append("<div style=\"margin-left:auto\"><span class=\"tag tag-empty\">Sorting</span></div>");
            sb.Append("</div>");
    ...
    private static string PalletQrSection(List<PalletInfo> pallets)
    {
        var sb = new StringBuilder();
        ...
        {
            var p = pallets[i];
            sb.Append("<div class=\"qr-card\">");
            sb.Append("<div class=\"qr-card-label\">🗂 QR Pallet #" + (i + 1) + "</div>");
            sb.Append("<div class=\"qr-card-wrap\">" + p.QrSvg + "</div>");
            sb.Append("<div class=\"qr-card-id\">" + p.PalletId + "</div>");
            var statusTag = p.Status == "Empty"
                ? "<span class=\"tag tag-empty\">Empty</span>"
                : "<span class=\"tag tag-occupied\">Occupied</span>";
            sb.Append("<div class=\"qr-card-pallet\">Zone: " + p.ZoneName + "<br/>" + statusTag + "</div>");
            sb.Append("</div>");
        }
        sb.Append("</div>");
        return sb.ToString();
    ...
    public static string BuildPreviewHtml(
        ...
        OriginInfo origin, HubInfo destination, List<SackDraft> sacks, List<PalletInfo> randomPallets, DateTime now)
    {
        ...
        // Pallet summary section
        body.Append("<div class=\"section-title\">🗂 Các mã Pallet để thử nghiệm (Quét trên web)</div>\n");
        body.Append(PalletQrSection(randomPallets));
    ...
    public static string BuildSuccessHtml(
        ...
        OriginInfo origin, HubInfo destination, List<PalletInfo> randomPallets, DateTime now)
    {
        var localNow    = now.AddHours(7);
        var body        = new StringBuilder();
        ...
        body.Append("<div class=\"sub\">1 trip Inbound · 3 sacks Sorting · Đích đến: " + destination.LocationName + " (" + destination.LocationId + ")</div>");
        body.Append("</div></div>\n");
        ...
        // Pallet summary section
        body.Append("<div class=\"section-title\">🗂 Các mã Pallet để thử nghiệm</div>\n");
        body.Append(PalletQrSection(randomPallets));
        ...
        for (int i = 0; i < sacks.Count; i++)
        {
            ...
            // Pallet QR in sack card removed
            body.Append("</div>");
        }
        body.Append("</div>\n");
