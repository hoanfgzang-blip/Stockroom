...
- Sau khi duyệt mới ghi vào database (trip + sack + trip_qr_token)
...
Người dùng xem xét → Console: [Y/N]?
     ↓
   [Y] → Ghi DB (trip + sack × 3 + qr_token) → Mở trang kết quả với ID thực
   [N] → Huỷ, không ghi gì
```
...
| `sack` × 3 | `InTransit` | s_destination=HUB Hà Nội, gắn với trip |
